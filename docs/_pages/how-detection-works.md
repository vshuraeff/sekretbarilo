...
`generic-high-entropy-value` bypasses most of them: the default stopwords, the hash suppression, the template handling and the documentation bonus all step aside for it. Only explicitly configured stopwords and per-rule value exceptions apply. This follows from what the rule is for — a suppression heuristic that a caller can trip by naming a variable well is not a safety property — and it is why that rule, more than any other, is the one people end up allowlisting.
