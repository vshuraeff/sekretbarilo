...
use common::{bin, setup_git_repo};

/// return the isolated user-layer Codex home under a fixture root.
fn codex_home(root: &std::path::Path) -> std::path::PathBuf {
    root.join("codex-home")
}

/// the fixture root with symlinks resolved. doctor derives the local Codex hook path from
/// `git rev-parse --show-toplevel`, which reports the resolved spelling, so a fixture that
/// serializes a path doctor also computes has to start from this root: on macOS a temp dir
/// lives under /var, a symlink to /private/var.
fn canonical_root(dir: &tempfile::TempDir) -> std::path::PathBuf {
    dir.path()
        .canonicalize()
        .expect("failed to canonicalize the fixture root")
}

/// run a subcommand without reading the user's actual Codex, Claude, or git configuration,
/// with every isolated path derived from `root`.
fn isolated_command_at(root: &std::path::Path, args: &[&str]) -> Command {
    let mut command = Command::new(bin());
    command
        .args(args)
        .env("GIT_CONFIG_GLOBAL", root.join(".fake-gitconfig"))
        .env("HOME", root.join("home"))
        .env("CODEX_HOME", codex_home(root))
...
fn isolated_command(dir: &tempfile::TempDir, args: &[&str]) -> Command {
    isolated_command_at(dir.path(), args)
}
...
fn e2e_doctor_warns_when_codex_hook_approval_is_disabled() {
  ...
  "hooks": {
    ...
    .unwrap();
    let codex_home = codex_home(&root);
    ...
    let output = isolated_command_at(&root, &["doctor"]).output().unwrap();
    let stderr = String::from_utf8_lossy(&output.stderr);
...
fn e2e_doctor_exit_0_when_all_installed() {
    ...
    // the installer adds the codex hook only when a codex cli is detected: an executable on
    // PATH, or an existing codex home directory. no codex binary is on PATH in this fixture,
    // so the directory has to exist before the first install, otherwise both installs skip
    // the codex hook and leave nothing for doctor to find.
    std::fs::create_dir_all(&codex_home).unwrap();

    // install both hooks (local + global)
    for args in [
        ["install", "all"].as_slice(),
        ["install", "all", "--global"].as_slice(),
    ] {
        let install = isolated_command_at(&root, args)
            .env("HOME", &root)
        ...
        assert!(
            ...
            "`sekretbarilo {}` should succeed, got:\n{}",
            args.join(" "),
    ...
    let hooks_json = std::fs::read_to_string(&hooks_json_path).unwrap_or_else(|error| {
        panic!(
            "install all should have written {}: {error}",
            hooks_json_path.display()
        )
    });
    assert!(
    ...
    let output = isolated_command_at(&root, &["doctor"])
        .env("HOME", &root)
        .env("CODEX_HOME", &codex_home)
        .env("PATH", &path_var)
        .output()
